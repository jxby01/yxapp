...
class NewsLogic{
	 ...
	 */
	public function news_list($page, $limit, $order){
		// $page = $page ? $page : 1;
		// $limit = $limit ? $limit : 10;
		// $order = $order ? $order : 'sort DESC';
		$rtn = $this->NewsLists->order($order)->page($page)->limit($limit)->select();
		if(!$rtn){
			$this->error = '没有更多数据';
			return false;
		}
	...
	public function count($sql){
		return (int)$this->NewsLists->where($sql)->count();
	}
}